    ...
    <div className="App">
      <Grid container>
        <Grid item md={1.7} className={days[1]} key={days[1].id}>
          <Typography className='DayContainer'>{days[0]}</Typography>
          {task.map((task, index) => (
...
/* for duplicate over a week
return (
    <div className="App">
      <Grid container>
          {days.map((day) => 
            <Grid item md={1.7} className={day} key={day.id}>
              <Paper>{day}</Paper>

              {task.map((task) => <Paper>{task.taskName}</Paper>)}

              <Button variant="outlined" key={day.id}
